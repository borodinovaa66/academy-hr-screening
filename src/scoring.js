...
function scoreSubmission(payload, config = defaultConfig) {
  ...
  const candidate = {
    ...
    portfolio: String(answers.portfolio || "").trim(),
  ...
  const total = Math.round((raw / rawMax) * 100);
  ...
  const strengths = makeStrengths(answers, score);
  const risks = makeRisks(answers, flags, score);
